...
var _ = Describe("cdi state", func() {
	var stateDir string
	var DontClean bool = false

	BeforeEach(func() {
		var err error
		stateDir, err = ioutil.TempDir("", "cdistate-")
	...
	Context("State API", func() {
		It("new file state", func() {
			...
			_, err = state.NewFileState("")
			Expect(err).To(HaveOccurred())

			_, err = state.NewFileState("/unknown/base/directory/")
			Expect(err).To(HaveOccurred())

			file, err := ioutil.TempFile("", "cdistate-file")
			Expect(err).NotTo(HaveOccurred())
			_, err = state.NewFileState(file.Name())
			os.Remove(file.Name()) //nolint: errcheck
			Expect(err).To(HaveOccurred())

			Expect(stateDir).ShouldNot(BeNil())
			fs, err := state.NewFileState(stateDir)
			Expect(err).NotTo(HaveOccurred())
		...
		It("concurrent read writes", func() {
			...
			for _, d := range data {
				wg.Add(1)
				// Writer
				go func(fs_ state.StateManager, d_ testData) {
