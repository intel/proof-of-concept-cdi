...
type nodeControllerServer struct {
	*DefaultControllerServer
	...
	volumes map[string]*nodeVolume // map of reqID:nodeVolume
	mutex   sync.Mutex             // lock for Volumes
}
...
func NewNodeControllerServer(nodeID string, dm dmanager.DeviceManager, sm state.StateManager) *nodeControllerServer {
...
func (cs *nodeControllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if err := dm.DeleteDevice(req.VolumeId, p.GetEraseAfter()); err != nil {
		if errors.Is(err, dmanager.ErrDeviceInUse) {
	...
	cs.mutex.Lock()
	defer cs.mutex.Unlock()
	delete(cs.volumes, req.VolumeId)
...
func (cs *nodeControllerServer) getVolumeByName(volumeName string) *nodeVolume {
	...
	defer cs.mutex.Unlock()
	for _, vol := range cs.volumes {
		if vol.Params[parameters.Name] == volumeName {
			return vol
		}
	}
	return nil
