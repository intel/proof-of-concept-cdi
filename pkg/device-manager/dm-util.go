...
func clearDevice(dev *DeviceInfo, flush bool) error {
	...
	if blocks == 0 {
		...
		if _, err := exec.RunCommand("shred", "-n", "1", dev.Path); err != nil {
		...
		if _, err := exec.RunCommand("dd", "if=/dev/zero", of, "bs=1024", count); err != nil {
...
func waitDeviceAppears(dev *DeviceInfo) error {
	for i := 0; i < 10; i++ {
